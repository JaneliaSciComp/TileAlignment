...
// Finish centroids and transform them to B-coords.
	...
	for( int i = 0; i < Ntrans; ++i ) {
		...
		fprintf(fout, "ABOverlay: Maps to (%f %f) in image B.\n",
		ctr[i].x, ctr[i].y );
	}
...
//	Raster32ToTifRGBA( "comp.tif", &raster2[0], w2, h2 );
  	if (comp_file != NULL) {
            sprintf(comp_name, comp_file);
        } else {
            sprintf(comp_name, "comp.png");
        }
	Raster32ToPngRGBA( comp_name, &raster2[0], w2, h2 );
}
...
// Read existing file...
        ...
        if (registered_file != NULL) {
            fout = stderr;
            sprintf(registered_name, registered_file);
        } else {
            sprintf(registered_name, "registered.png");
        }
	if( f = fopen(registered_name, "r" ) ) {
		...
		bpix = Raster8FromPng( registered_name, w2, h2 );
	}
	else {
...
// Normalize and store new values
	...
	Raster8ToPng8( registered_name, bpix, w, h );
